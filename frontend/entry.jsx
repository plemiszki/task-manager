...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (topBarNode) {
    createRoot(topBarNode).render(<TopBar />);
  }

  const tasksIndexNode = document.getElementById("tasks-index");
  if (tasksIndexNode) {
    createRoot(tasksIndexNode).render(<TasksIndex />);
  }

  const futureTasksIndexNode = document.getElementById("future-tasks-index");
  if (futureTasksIndexNode) {
    createRoot(futureTasksIndexNode).render(<FutureTasksIndex />);
  }

  const recurringTasksIndexNode = document.getElementById(
    "recurring-tasks-index",
  );
  if (recurringTasksIndexNode) {
    createRoot(recurringTasksIndexNode).render(<RecurringTasksIndex />);
  }

  const recurringTaskDetailsNode = document.getElementById(
    "recurring-task-details",
  );
  if (recurringTaskDetailsNode) {
    createRoot(recurringTaskDetailsNode).render(<RecurringTaskDetails />);
  }

  const recipesIndex = document.getElementById("recipes-index");
  if (recipesIndex) {
    createRoot(recipesIndex).render(<RecipesIndex />);
  }
  ...
  if (scheduleNode) {
    createRoot(scheduleNode).render(<Schedule />);
  }
